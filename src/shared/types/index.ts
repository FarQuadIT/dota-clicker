...
/**
 * Интерфейс для ежедневного задания
 * Описывает структуру задания из ответа API
 */
export interface Quest {
  ...
  /** Текущее значение прогресса по заданию */
  questCurrentValue: number;
  
  /** Цель задания (null означает, что задание не имеет конкретной цели) */
  questGoal: number | null;
  
  /** Флаг получения награды (true если награда уже получена) */
  claimedReward: boolean;
}

/**
 * Интерфейс для ответа API со списком заданий
 */
export interface QuestsResponse {
  /** ID пользователя */
  userId: string;
  
  /** Массив заданий пользователя */
  quests: Quest[];
}

/**
 * Интерфейс для обновления отдельного квеста
 */
export interface QuestUpdate {
  /** Уникальный идентификатор задания */
  questId: number;
  
  /** Текущее значение прогресса (опционально) */
  currentValue?: number;
  
  /** Флаг получения награды (опционально) */
  claimedReward?: boolean;
}

/**
 * Интерфейс для запроса обновления квестов
 */
export interface UpdateQuestsPayload {
  /** ID пользователя */
  userId: number;
  
  /** Массив обновлений квестов */
  quests: QuestUpdate[];
}
